...
export default function PrintPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setItems(data.map((g) => ({
        ...g,
        qrUrl: `${baseUrl}/style/${encodeURIComponent(g.styleRef)}`,
      })));
    } catch {
      ...
      <div ref={printAreaRef} className="print-only" style={{ display: 'none' }}>
        {pages.map((page, pi) => (
          <div key={pi} className="qr-print-grid" style={{ pageBreakAfter: pi < pages.length - 1 ? 'always' : 'auto' }}>
            {page.map((item) => (
              <div key={item.styleRef} className="qr-print-item">
                <QRCodeSVG
                  value={item.qrUrl}
                  size={180}
                  level="M"
                ...
                <div className="qr-print-label">
                  <div className="style-ref">{item.styleRef}</div>
